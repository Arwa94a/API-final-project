...
const { User, signupJoi, loginJoi, profileJoi, forogotPasswordJoi, resetPasswordJoi } = require("../models/User")
...
router.put("/profile", checkTokenUser, validteBody(profileJoi), async (req, res) => {
  try {
    ...
    const user = await User.findByIdAndUpdate(
      req.userId,
      { $set: { firstName, lastName, email, password: hash, photo, username } },
      { new: true }
    )
    res.json(user)
  } catch (error) {
...
router.delete("/profile/:id", checkTokenUser, async (req, res) => {
  try {
...
router.delete("/seller/:id", checkAdmin, async (req, res) => {
  try {
    const user = await UserSeller.findById(req.params.id)
    if (!user) return res.status(404).send("user is not found")
    if (user.roler === "Admin") return res.status(403).send("unauthorized action")
    await UserSeller.findByIdAndRemove(req.params.id)
    res.send("user removed")
  } catch (error) {
    console.log(error.message)
    res.status(500).send(error.message)
...
router.put("/:orderId", checkTokenUser, validteBody(orderJoi), validateId("orderId"), async (req, res) => {
  try {
    const { quantity } = req.body
    const product = await Product.find()
    if (!product) return res.status(404).send("product is not  found")

    const order = await Order.findByIdAndUpdate(req.params.orderId, { $set: { quantity } }, { new: true })
    res.json(order)
  } catch (error) {
    console.log(error.message)
    res.status(500).send(error.message)
